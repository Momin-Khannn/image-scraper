...
def test_build_public_gallery_copies_images_and_writes_latest_page(tmp_path: Path):
    ...
    assert (site_dir / "requests" / "index.html").exists()
...
def test_build_public_gallery_can_publish_issue_subdir_and_latest_alias(tmp_path: Path):
    docs_dir = tmp_path / "docs"
    output_dir = tmp_path / "output"
    site_dir = tmp_path / "site"
    docs_dir.mkdir()
    ...
    report = build_public_gallery(
        ...
        output_dir,
        site_dir,
        gallery_subdir="requests/issue-7",
        preserve_site=True,
    )

    assert report["gallery_dir"].endswith("requests\\issue-7") or report["gallery_dir"].endswith("requests/issue-7")
    assert (site_dir / "requests" / "issue-7" / "index.html").exists()
    assert (site_dir / "latest" / "index.html").exists()
    requests_html = (site_dir / "requests" / "index.html").read_text(encoding="utf-8")
    assert "Issue 7" in requests_html
